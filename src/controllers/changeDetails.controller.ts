...
export class ChangeDetailsController extends JourneyBaseController {
    ...
    public async get(): Promise<string> {
        ...
        const signatory = session.isFromCheckAnswers
            ? this.getSignatoryFromSession(session)
    ...
    private withUpdatedDefineSignatoryInfoForm(
        ...
        const isOnBehalf =
            !!defineSignatoryInfoFormModel[onBehalfNameKey] && defineSignatoryInfoFormModel[onBehalfNameKey] !== "";
    ...
    private getSignatoryFromSession(session: DissolutionSession) {
        ...
        const directorToEdit: DirectorToSign | undefined = directorsToSign.find(
            director => director.id === session.signatoryIdToEdit
        );
        if (!directorToEdit) {
