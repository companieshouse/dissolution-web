...
export class ApplicationStatusController extends JourneyBaseController {
    ...
    public async change(
        @requestParam("signatoryId") signatoryId: string,
        @queryParam("check_answers") isFromCheckAnswers: string
    ...
    public async resend(@requestBody() body: ResendEmailFormModel): Promise<RedirectResult> {
        ...
        const reminderSent: boolean = await this.dissolutionService.sendEmailNotification(
            companyNumber,
            signatoryEmail
        );
