...
export class DefineSignatoryInfoController extends JourneyBaseController {
    ...
    public async post(@requestBody() body: DefineSignatoryInfoFormModel): Promise<string | RedirectResult> {
        ...
        const errors: Optional<ValidationErrors> = this.validator.validate(
            body,
            defineSignatoryInfoSchema(viewableSignatories)
        );
        if (errors) {
    ...
    private updateSession(session: DissolutionSession, body: DefineSignatoryInfoFormModel): void {
        const updatedSignatories = this.signatoryService.updateSignatoriesWithContactInfo(
            session.directorsToSign || [],
            body
        );
