...
export default function selectDirectorSchema(
    ...
    (directors || [])
        ...
        .forEach(d => {
            ...
            const fieldSchema = Joi.string().when("director", {
                is: d.id,
...
function onBehalfNameValidationSchema() {
    return Joi.string().required().max(250).pattern(/\S/, { name: "non-whitespace" }).messages({
