...
const invalidPresenterAuthCodeError: string =
    "Presenter authentication code must only include letters a to z, and numbers";
...
const payByAccountDetailsSchema = Joi.object({
    presenterId: Joi.string().required().pattern(/^\d+$/).length(11).messages({
    ...
    presenterAuthCode: Joi.string().required().alphanum().length(11).messages({
    ...
    _csrf: Joi.string().optional().messages({
