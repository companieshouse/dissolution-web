...
describe("Change Details Schema", () => {
    describe("Standard signatory", () => {
        it("should return no errors when data is valid", () => {
            const signatory: DissolutionGetDirector = aDissolutionGetDirector()
                .withName("Mr Standard Director Signatory")
                .build();
            const form: ChangeDetailsFormModel = aChangeDetailsFormModel()
                .withDirectorEmail("director@mail.com")
                .build();
        ...
        it("should return an error if mandatory text field have not been provided", () => {
            const signatory: DissolutionGetDirector = aDissolutionGetDirector()
                .withName("Mr Standard Director Signatory")
                .build();
            const form: ChangeDetailsFormModel = aChangeDetailsFormModel().withDirectorEmail("").build();
            ...
            assert.equal(
                ...
                `Enter the email address for Mr Standard Director Signatory`
            );
        });
        ...
        it("should return an error if email field does not contain a valid email value", () => {
            const signatory: DissolutionGetDirector = aDissolutionGetDirector()
                .withName("Mr Standard Director Signatory")
                .build();
            const form: ChangeDetailsFormModel = aChangeDetailsFormModel().withDirectorEmail("non valid email").build();
    ...
    describe("Corporate Signatory", () => {
        it("should return no errors when data is valid", () => {
            const signatory: DissolutionGetDirector = aDissolutionGetDirector()
                .withName("Mr Corporate Director Signatory")
                .withOnBehalfName("Mr Old Accountant")
                .build();
            const form: ChangeDetailsFormModel = aChangeDetailsFormModel()
                .withDirectorEmail(undefined)
                .withOnBehalfName("Mr New Accountant")
                .withOnBehalfEmail("new.accountant@mail.com")
                .build();
        ...
        it("should return an error if mandatory text fields have not been provided", () => {
            ...
            assert.equal(
                errors.error!.details[0].message,
                `Enter the name of the authorised person who will sign for Mr Corporate Director Signatory`
            );

            assert.equal(errors.error!.details[1].context!.key, `onBehalfEmail`);
            assert.equal(errors.error!.details[1].type, `string.empty`);
            assert.equal(
                errors.error!.details[1].message,
                `Enter the email address for the authorised person who will sign for Mr Corporate Director Signatory`
            );
        });
        ...
        it("should return an error if email field does not contain a valid email value", () => {
            ...
            const form: ChangeDetailsFormModel = aChangeDetailsFormModel()
                ...
                .withOnBehalfEmail("not an email")
                .build();
        ...
        it("should return an error if a name is provided that is above the maximum length", () => {
            const signatory: DissolutionGetDirector = aDissolutionGetDirector()
                .withName("Mr Corporate Signatory")
                .withOnBehalfName("Mr Old Accountant")
                .build();
            const form: ChangeDetailsFormModel = aChangeDetailsFormModel()
                .withDirectorEmail(undefined)
            ...
            assert.equal(
                errors.error!.details[0].message,
                `Name of authorised person signing for Mr Corporate Signatory must be 250 characters or less`
            );
        });
    });
});
