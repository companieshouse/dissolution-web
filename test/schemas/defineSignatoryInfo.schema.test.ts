...
describe("Define Signatory Info Schema", () => {
    ...
    it("should return no errors when data is valid", () => {
        const signatory1: DirectorToSign = aDirectorToSign()
            .withId(SIGNATORY_1_ID)
            .withName("Standard Signatory")
            .withOfficerRole(OfficerRole.DIRECTOR)
            .build();
        const signatory2: DirectorToSign = aDirectorToSign()
            .withId(SIGNATORY_2_ID)
            .withName("Corporate Signatory")
        ...
        const errors: ValidationResult = defineSignatoryInfoSchema([signatory1, signatory2]).validate(form, {
            abortEarly: false,
        });
    ...
    it("should return an error if mandatory text fields have not been provided", () => {
        ...
        assert.equal(
            ...
            `Enter the name of the authorised person who will sign for Mr Corporate Signatory`
        );
        ...
        assert.equal(
            errors.error!.details[2].message,
            `Enter the email address for the authorised person who will sign for Mr Corporate Signatory`
        );
    });
