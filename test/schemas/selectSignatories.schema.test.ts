...
describe('Select Signatories Schema', () => {
  ...
  it('should return no errors when data is valid', () => {
    ...
    const errors: ValidationResult = selectSignatoriesSchema(1).validate(validForm)

    assert.isUndefined(errors.error)
  })

  it('should return an error when no signatories are provided', () => {
    const invalidForm: SelectSignatoriesFormModel = generateSelectSignatoriesFormModel()
    invalidForm.signatories = undefined

    const errors: ValidationResult = selectSignatoriesSchema(3).validate(invalidForm)
  ...
  it('should return an error when the minimum number of signatories is not provided', () => {
