...
describe("EndorseCompanyClosureCertificateController", () => {
    ...
    const CONFIRMATION_ERROR_CORPORATE =
    ...
    describe("GET request", () => {
        it("should render confirmation checkbox hint for LLP corporate member", () => {
            ...
            return request(initApp())
                ...
                .then(res => {
                    assert.isTrue(
                        res.text.includes(HINT_TEXT_MEMBER),
                        "Expected confirmation checkbox hint to be present for LLP corporate member"
                    );
                });
        });
        it("should render endorse certificate page without an on behalf statement if director is signing and not a corporate officer", () => {
            ...
            return request(initApp())
                ...
                .then(res => {
                    ...
                    assert.isTrue(htmlAssertHelper.hasText("span#confirmationLabel", CONFIRMATION_LABEL_DIRECTOR));
                    assert.equal(
                        htmlAssertHelper.getAttributeValue(".govuk-back-link", "href"),
                        `${VIEW_COMPANY_INFORMATION_URI}?companyNumber=${dissolutionSession.approval!.companyNumber}`
                    );
                });
        });
        ...
        it("should not render confirmation checkbox hint for non-corporate officer", () => {
            ...
            return request(initApp())
                ...
                .then(res => {
                    ...
                    assert.isFalse(
                        res.text.includes(HINT_TEXT_DIRECTOR),
                        "Expected confirmation checkbox hint to be absent for non-corporate officer"
                    );
                });
        });
        ...
        it("should render confirmation checkbox hint for corporate director", () => {
            ...
            return request(initApp())
                ...
                .then(res => {
                    ...
                    assert.isTrue(
                        res.text.includes(HINT_TEXT_DIRECTOR),
                        "Expected confirmation checkbox hint to be present for corporate director"
                    );
                });
        });
        ...
        it("should render both confirmation and declaration checkboxes and new headings", () => {
            ...
            return request(initApp())
                ...
                .then(res => {
                    ...
                    assert.isTrue(
                        htmlAssertHelper.anyTagHasText(
                            "h2",
                            "By ticking both of the boxes below you are signing the application."
                        ),
                        "Expected to find the correct tickboxes heading in an <h2> tag"
                    );
        ...
        it("should render endorse certificate page with corporate officer confirmation label", async () => {
            ...
            it("should render endorse certificate page with members for LLDS01", async () => {
                ...
                assert.isTrue(
                    htmlAssertHelper.hasText("span#confirmationLabel", CONFIRMATION_LABEL_LLP_MEMBER),
                    "Expected confirmation label for LLP member"
                );
                assert.isTrue(htmlAssertHelper.hasText("#declaration-heading", "Declaration"));
