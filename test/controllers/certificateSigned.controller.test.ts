...
beforeEach(() => {
    ...
    when(viewApplicationStatusMapper.mapToViewModel(dissolutionSession, dissolution, false)).thenReturn(
    ...
    describe("GET request", () => {
        it("should render the CertificateSigned page with director text when the company is plc", async () => {
            ...
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#signatures",
                    "We need to receive signatures from all signing directors before the person making the application can pay for and submit it. It may take some time for others to sign. You can check who has signed below."
                )
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#parties",
                    "The directors must send this to all interested parties within 7 days of the application being submitted."
                )
            );
        });
        ...
        it("should render the CertificateSigned page with member text if the company is LLP", async () => {
            ...
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#signatures",
                    "We need to receive signatures from all signing members before the person making the application can pay for and submit it. It may take some time for others to sign. You can check who has signed below."
                )
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#parties",
                    "The members must send this to all interested parties within 7 days of the application being submitted."
                )
            );
        });
    });
