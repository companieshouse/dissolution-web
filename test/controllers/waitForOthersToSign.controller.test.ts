...
beforeEach(() => {
    ...
    when(viewApplicationStatusMapper.mapToViewModel(dissolutionSession, dissolution, true)).thenReturn(
...
describe("WaitForOthersToSignController", () => {
    describe("GET request", () => {
        it("should render the WaitForOthers page with director text when company is plc", async () => {
            ...
            assert.isTrue(
                htmlAssertHelper.hasText("h1", "The directors must sign the application before you can submit it")
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#email",
                    "We have emailed the directors, or those signing on their behalf, asking them to sign the application. It may take some time for them to do this."
                )
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#signed",
                    "When all directors have signed, we will email you with instructions to pay for and submit the application."
                )
            );
        });
        ...
        it("should render the WaitForOthers page with director text when company is single director plc", async () => {
            ...
            assert.isTrue(
                htmlAssertHelper.hasText("h1", "The director must sign the application before you can submit it")
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#email",
                    "We have emailed the director, or someone signing on their behalf, asking them to sign the application. It may take some time for them to do this."
                )
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#signed",
                    "When the director has signed, we will email you with instructions to pay for and submit the application."
                )
            );
        });
        ...
        it("should render the WaitForOthers page with member text when company is llp", async () => {
            ...
            assert.isTrue(
                htmlAssertHelper.hasText("h1", "The members must sign the application before you can submit it")
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#email",
                    "We have emailed the members, or those signing on their behalf, asking them to sign the application. It may take some time for them to do this."
                )
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#signed",
                    "When all members have signed, we will email you with instructions to pay for and submit the application."
                )
            );
        });
        ...
        it("should render the WaitForOthers page with member text when company is single member llp", async () => {
            ...
            assert.isTrue(
                htmlAssertHelper.hasText("h1", "The member must sign the application before you can submit it")
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#email",
                    "We have emailed the member, or someone signing on their behalf, asking them to sign the application. It may take some time for them to do this."
                )
            );
            assert.isTrue(
                htmlAssertHelper.hasText(
                    "#signed",
                    "When the member has signed, we will email you with instructions to pay for and submit the application."
                )
            );
        });
        ...
        describe("View Application Status", () => {
            ...
            describe("change", () => {
                it("should display the change link beside each editable signatory", async () => {
                    ...
                    assert.equal(
                        htmlAssertHelper.getAttributeValue("#change-email-0", "href"),
