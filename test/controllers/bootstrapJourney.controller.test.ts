...
describe("BootstrapJourneyController", () => {
    ...
    it("when authenticated user is NOT authorised for requested company then redirects to account auth", async () => {
        ...
        when(companyAuthServiceMock.issueAuthRedirectUri(anything(), anything())).thenReturn(
            Promise.resolve("http://account.chs-dev/oauth2/authorise?state=encoded")
        );
    ...
    invalidCompanyNumberCases.forEach(tc => {
        it(`applicationNumber invalid: ${tc.desc}`, async () => {
            ...
            const res = await request(app).get(BOOTSTRAP_JOURNEY_URI).query({ companyNumber: tc.input });
