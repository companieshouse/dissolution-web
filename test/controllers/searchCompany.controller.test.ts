    ...
    describe("GET request", () => {
        it("should redirect to company lookup", async () => {
            ...
            const expectedRedirect =
                "/company-lookup/search?forward=/close-a-company/bootstrap-journey?companyNumber=%7BcompanyNumber%7D";

            const res = await request(app).get(SEARCH_COMPANY_URI).expect(StatusCodes.MOVED_TEMPORARILY);
