...
describe("HowDoYouWantToPayController", () => {
    ...
    beforeEach(() => {
        ...
        when(paymentService.generatePaymentURL(TOKEN, anything(), anything(), anything())).thenResolve(
            REDIRECT_CARD_URL
        );
    ...
    describe("GET", () => {
        ...
        it("should display the how do you want to pay page if the application has not been paid for", async () => {
            ...
            const res = await request(app).get(HOW_DO_YOU_WANT_TO_PAY_URI).expect(StatusCodes.OK);
    ...
    describe("POST", () => {
        it("should display how do you want to pay page with validation error when no payment type is selected", async () => {
            ...
            const error: ValidationErrors = generateValidationError(
                "paymentType",
            ...
            const res = await request(app).post(HOW_DO_YOU_WANT_TO_PAY_URI).send(form).expect(StatusCodes.BAD_REQUEST);

            const htmlAssertHelper: HtmlAssertHelper = new HtmlAssertHelper(res.text);

            assert.isTrue(
                htmlAssertHelper.containsText(".govuk-error-summary__body", "Select how the payment will be made")
            );
        });
        ...
        it("should redirect to the credit card payment page when the credit/debit card payment type is selected", async () => {
            ...
            const sessionCaptor: ArgCaptor2<Request, DissolutionSession> = capture<Request, DissolutionSession>(
                sessionService.setDissolutionSession
            );
