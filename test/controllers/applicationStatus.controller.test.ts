    ...
    describe("Send-email", () => {
        ...
        sendEmailCases.forEach(testCase => {
            it(`attempts to send email and records reminder attempt - ${testCase.name}`, async () => {
                ...
                when(sessionService.requireDissolutionCompanyNumber(anything())).thenReturn(
                    dissolutionSession.companyNumber!
                );
                when(
                    dissolutionService.getDissolutionSignatoryEmail(
                        anything(),
                        dissolutionSession.companyNumber!,
                        signatoryId
                    )
                ).thenResolve(signatoryEmail);
                when(
                    dissolutionService.sendEmailNotification(dissolutionSession.companyNumber!, signatoryEmail)
        ...
        validSignatoryIds.forEach(body => {
            it(`accepts valid signatory id: ${JSON.stringify(body)}`, async () => {
                ...
                when(
                    dissolutionService.getDissolutionSignatoryEmail(anything(), dissolutionSession.companyNumber!, id)
                ).thenResolve(signatoryEmail);
                when(
                    dissolutionService.sendEmailNotification(dissolutionSession.companyNumber!, signatoryEmail)
                ).thenResolve(true);
        ...
        const noEmailFoundCases = [
            ...
            {
                name: "directors present but id not found",
