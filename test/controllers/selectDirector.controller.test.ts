...
describe("SelectDirectorController", () => {
    ...
    const CORPORATE_DIRECTOR_HINT_TEXT =
        "You must be authorised by this corporate director to sign the application on its behalf.";
    const CORPORATE_MEMBER_HINT_TEXT =
        "You must be authorised by this corporate member to sign the application on its behalf.";
    ...
    describe("POST - ensure form submission is handled correctly", () => {
        ...
        describe("session", () => {
            it("should store the form in session if validation passes", async () => {
                const form: SelectDirectorFormModel = aSelectDirectorFormModel()
                    .withDirector(DIRECTOR_1_ID)
                    .withOnBehalfName(DIRECTOR_1_ID, "Authorised person")
                    .build();
                ...
                when(
                    validator.validate(
                        deepEqual(form),
                        deepEqual(selectDirectorSchema(dissolutionSession.officerType!))
                    )
                ).thenReturn(null);
            ...
            describe("Applicant is a director", () => {
                ...
                it("should store the director details with an onBehalfName when corporate director selected", async () => {
                    ...
                    const directors = [
                        aDirectorDetails()
                            .withId(corporateDirectorId)
                            .withName(corporateDirectorName)
                            .withOfficerRole(OfficerRole.CORPORATE_DIRECTOR)
                            .build(),
                    ];

                    const form: SelectDirectorFormModel = aSelectDirectorFormModel()
                        .withDirector(corporateDirectorId)
                        .withOnBehalfName(corporateDirectorId, "Gus")
                        .build();
