...
describe("asDirectorSingleMultiConfirmationGoalId", () => {
    it(`should return 'multiDirectorConfirmationGoalId' when isMultiDirector is true`, () => {
        assert.equal(
            piwikConfig.multiDirectorConfirmationGoalId,
            asDirectorSingleMultiConfirmationGoalId(true, piwikConfig)
        );
    });

    it(`should return 'limitedCompanyGoalId' when isMultiDirector is false`, () => {
        assert.equal(
            piwikConfig.singleDirectorConfirmationGoalId,
            asDirectorSingleMultiConfirmationGoalId(false, piwikConfig)
        );
    });
});
