...
describe("asApplicationTypeConfirmationGoalId", () => {
    it(`should return 'partnershipConfirmationGoalId' when company is of type partnership`, () => {
        assert.equal(
            piwikConfig.partnershipConfirmationGoalId,
            asApplicationTypeConfirmationGoalId(ApplicationType.LLDS01, piwikConfig)
        );
    });

    it(`should return 'limitedCompanyConfirmationGoalId' when company is of type limited company`, () => {
        assert.equal(
            piwikConfig.limitedCompanyConfirmationGoalId,
            asApplicationTypeConfirmationGoalId(ApplicationType.DS01, piwikConfig)
        );
    });
});
