...
describe("ApprovalService", () => {
    ...
    beforeEach(() => {
        ...
        service = new ApprovalService(instance(companyOfficersService), instance(mapper));
    });
    ...
    it("should fall back to API when session directors are not available", async () => {
        ...
        when(
            companyOfficersService.isCorporateOfficer(TOKEN, dissolution.company_number, signatory.officer_id)
        ).thenResolve(true);
        when(mapper.mapToApprovalModel(dissolution, signatory, anything())).thenReturn(approval);
