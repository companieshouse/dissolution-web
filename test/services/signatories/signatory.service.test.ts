    ...
    describe("updateSignatoriesWithContactInfo", () => {
        ...
        it("should update signatories with their contact info for a standard officer", () => {
            const contactForm: DefineSignatoryInfoFormModel = aDefineSignatoryInfoForm()
                ...
                .withDirectorEmail(SIGNATORY_2_ID_LOWER, "otherdirector@mail.com")
                .build();

            const signatory1: DirectorToSign = aDirectorToSign()
                .withId(SIGNATORY_1_ID_LOWER)
                .withName("Mr Standard Director 1")
                .build();
            const signatory2: DirectorToSign = aDirectorToSign()
                .withId(SIGNATORY_2_ID_LOWER)
                .withName("Mr Standard Director 2")
                .build();
        ...
        it("should update signatories with their contact info for corporate officer", () => {
            const contactForm: DefineSignatoryInfoFormModel = aDefineSignatoryInfoForm()
                ...
                .withOnBehalfEmail(SIGNATORY_2_ID_LOWER, "solicitor@mail.com")
                .build();

            const signatory1: DirectorToSign = aDirectorToSign()
                .withId(SIGNATORY_1_ID_LOWER)
                .withName("Corporate Director 1")
                .withOfficerRole(OfficerRole.CORPORATE_DIRECTOR)
                .build();
            const signatory2: DirectorToSign = aDirectorToSign()
                .withId(SIGNATORY_2_ID_LOWER)
                .withName("Corporate Director 2")
                .withOfficerRole(OfficerRole.CORPORATE_NOMINEE_DIRECTOR)
                .build();
        ...
        it("should handle multiple signatories selecting different options for signing preference", () => {
            ...
            const signatory1: DirectorToSign = aDirectorToSign()
                .withId(SIGNATORY_1_ID_LOWER)
                .withName("Standard Director")
                .withOfficerRole(OfficerRole.DIRECTOR)
                .build();
            const signatory2: DirectorToSign = aDirectorToSign()
                .withId(SIGNATORY_2_ID_LOWER)
                .withName("Corporate Director")
                .withOfficerRole(OfficerRole.CORPORATE_DIRECTOR)
                .build();
        ...
        it("should return the original signatory unchanged if no contact info is present", () => {
            const contactForm: DefineSignatoryInfoFormModel = aDefineSignatoryInfoForm().build();

            const signatory: DirectorToSign = aDirectorToSign().withId("789").withName("Applicant Director").build();

            const updated = service.updateSignatoriesWithContactInfo([signatory], contactForm);

            assert.deepEqual(updated[0], signatory);
        });
    });
});
